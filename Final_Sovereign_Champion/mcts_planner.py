...
NASUTA_ROOT = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "engine")
...
def guided_rollout_wrapper(self_wrapper):
    ...
    try:
        ...
        while not d_done and steps < 30:
            ...
            # Dynamic heuristic variables
            dyn_qol_limit = int(os.environ.get("TUNER_MCTS_QOL_LIMIT", 15))
            dyn_pol_limit = int(os.environ.get("TUNER_MCTS_POL_LIMIT", 10))
            dyn_env_limit = int(os.environ.get("TUNER_MCTS_ENV_LIMIT", 15))
            dyn_edu_limit = int(os.environ.get("TUNER_MCTS_EDU_LIMIT", 15))

            # HEURISTIC SELECTION (Survival-First Sovereign Logic)
            if avail > 0:
                # 1. CRITICAL PROTECTION: Quality of Life (Protects Politics/Stability)
                if 5 in valid_actions and V[3] < dyn_qol_limit: move = 5
                # 2. SYSTEMIC STABILITY: Production (Protects Economy/Politics)
                elif 2 in valid_actions and V[7] < dyn_pol_limit: move = 2
                # 3. ENVIRONMENTAL RECOVERY: Sanitation
                elif 1 in valid_actions and V[5] < dyn_env_limit: move = 1
                # 4. EDUCATION (Long-term)
                elif 4 in valid_actions and V[2] < dyn_edu_limit: move = 4
                else: 
                    # If stable, diversify
