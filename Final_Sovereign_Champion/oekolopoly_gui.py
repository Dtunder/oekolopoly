...
class SovereignGuardian:
    ...
    def get_final_action(self, avail):
        ...
        # 0. PREDICTIVE PRUNING (Black Sky Shield)
        if V[5] <= 9 or V[7] < -5:
            d = min(avail, 5)
            dist[1] += d
            avail -= d
            reasons.append("[PREDICTIVE PRUNING] Forcing Production to dump QoL/prevent collapse.")
        ...
        # 3. ALCHEMIST BURN (Avoid 35 limit)
        while avail + int(V[9]) > 28:
            ...
            if int(V[2]) + dist[2] < 29:
                ...
                if int(V[1]) + dist[1] < 18:
                    ...
                    reasons.append("Burn: Increasing Prod.")
                else: break
            elif V[5] > 20:
